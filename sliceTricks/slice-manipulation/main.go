...
func main() {
    // 3 basic oprations: slicing, append, copy
    // slicing will modify the slice header
    // append only adds new element to the position index len of the slice, and change the underlying array if there is no more space for len
    // copy will move the elements from one slice to another
    // simple slicing: b := a[i:j]
    // ptr(b) = ptr(a) + i
    // len(b) = j - i
    // cap(b) = cap(a) - i
    // the left bound will affect the ptr and cap, and the right bound will affect the len
    // full slicing expression: b := a[i:j:k]
    // ptr(b) = ptr(a) + i
    // len(b) = j - i
    // cap(b) = k - i
    // comparing to the simple one, full expression allows you to limit the cap of slicing result
	a := make([]int, 5)
	b := make([]int, 5)
	// append a slice
	a = append(a, b...)
